...
{
	$yenisifre 			= trim(substr($_REQUEST["parola1"],0,32));
	$yenisifredogrulama = trim(substr($_REQUEST["parola2"],0,32));
	$mevcutsifre 		= trim(substr($_REQUEST["parolam"],0,32));
	...
	if ($islem_bilgisi == '')
	{
		...
		{
			$vt->sql('UPDATE rss_user SET user_password = %s WHERE user_email = %s AND user_password = %s');
